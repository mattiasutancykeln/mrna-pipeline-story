...
(function () {
  ...
  function orientGroove(v) {
    // TCR's-eye view: look into the groove from the presenting face
    v.zoomTo(); v.rotate(90, "x"); v.rotate(90, "y"); v.rotate(180, "x"); v.render();
    const start = v.getView();
    v.zoomTo({ chain: "C" }, 0); v.zoom(0.62); v.render();
  ...
  function orientDockSequence(v) {
    // K0 — presentation: look straight into the groove, framed on the peptide
    //      (the TCR, above the MHC, falls outside this tight frame)
    ...
    v.zoomTo(); v.rotate(-90, "x"); v.zoom(1.08); v.render();
    const k2 = v.getView();
  ...
  function orientProtein(v) {
    ...
    v.zoomTo({ chain: "A", resi: PEP_WIN }, 0); v.zoom(0.62); v.render();
    return { keys: [start, v.getView()] };
  }
  ...
  function orientDNA(v) {
    ...
    v.zoomTo({}); v.rotate(90, "x"); v.rotate(25, "y"); v.zoom(0.52); v.render();
    const start = v.getView();
    ...
    v.zoomTo({ chain: "A", resi: "5-9" }, 0); v.zoom(0.62); v.render();
    return { keys: [start, v.getView()] };
  }
  ...
  window.StructureScene = function (containerId, key, kind) {
    ...
    load.then((txt) => {
      ...
      keys = cfg.orient(v).keys;
      v.setView(keys[0]); v.render(); ready = true;
    });
    return {
      setProgress(p) { if (!ready) return; if (cfg.reveal) cfg.reveal(v, p, state);
        v.setView(lerpKeys(keys, p)); v.render(); },
      resize() { if (ready) { v.resize(); v.render(); } },
      isReady: () => ready,
    };
  };
  // back-compat
